rv.controllers.global = function($scope, $location, ravennaCometdService) {
    ...
    ravennaCometdService.subscribe('/ravenna/stream/errors', function(message) {
        $scope.$emit('notify', {text: message.data});
    });
    ...
    listeners.push(ravennaCometdService.addSettingsListener(
        function(data) { // test function
            return (data.identity  &&  data.identity._version)  ||  data._capabilities  ||  data.routing;
        },
...
rv.controllers.global.$inject = ['$scope', '$location', 'ravennaCometdService'];
